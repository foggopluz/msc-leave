...
import { rejectLeaveRequest, leaveRequests } from '@/lib/store'
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updated = rejectLeaveRequest(id, approver_id, stage as ApprovalStage, comment)
  if (!updated) return NextResponse.json({ error: 'Failed to reject' }, { status: 500 })
